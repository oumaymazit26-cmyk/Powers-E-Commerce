...
async function editProduct(id) {
    try {
        ...
        if (p.gallery) {
            const galleryImages = p.gallery.split(',').map(g => g.trim()).filter(g => g);
            galleryImages.forEach((imgName, idx) => {
                if (idx < 4) {
                    const slot = idx + 1;
                    const img = document.getElementById(`gallery-preview-${slot}`);
                    const imgUrl = imgName.startsWith('http') ? imgName : `${API}/uploads/${imgName}`;
                    img.src = imgUrl;
                    img.style.display = 'block';
                    const icon = document.querySelector(`.gallery-slot[data-slot="${slot}"] i`);
